...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        next_place, distance = self.place.entrance, 1
        while next_place.name != hive.name:
            if next_place.bees and distance >= self.min_range and distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # END Problem 5
        self.armor -= amount
        if self.armor <= 0 and self.place.bees:
            for b in self.place.bees:
                b.armor = b.armor - self.damage
            for b in self.place.bees[:]:
                if b.armor <= 0:
...
# END Problem 8
class WallAnt(Ant):
    """WallAnt does nothing each turn but block bees"""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            k = self.place.exit
            while k:
                if k.ant and k.ant not in self.doubled:
                    k.ant.damage = 2 * k.ant.damage
                    self.doubled.append(k.ant)
                if isinstance(k.ant, BodyguardAnt) or isinstance(k.ant, TankAnt):
                    if k.ant.ant and (k.ant.ant not in self.doubled):
                        k.ant.ant.damage = 2 * k.ant.ant.damage
                        self.doubled.append(k.ant.ant)
                k = k.exit
        else:
